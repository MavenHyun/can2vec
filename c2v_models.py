...
class FarSeer:
    ...
    def re_constructor(self, target, fun, toggle):
    ...
    def SurvivalNet(self, n_hid):
        with tf.name_scope("Survival_Net_parameters"):
            ...
            lasso_alpha = tf.placeholder(tf.float32)
        ...
        with tf.name_scope("Survival_Net_layers"):
            ...
            final_product = final_sum * self.data.T['cen']
            cost = -tf.reduce_sum(final_product) \
            + lasso_alpha * tf.reduce_sum(penalty_lambda * tf.nn.l2_loss(w_6)) \
            + lasso_alpha * tf.reduce_sum(penalty_lambda * tf.nn.l2_loss(w_5)) \
            + lasso_alpha * tf.reduce_sum(penalty_lambda * tf.nn.l2_loss(w_4)) \
            + lasso_alpha * tf.reduce_sum(penalty_lambda * tf.nn.l2_loss(w_3)) \
            + lasso_alpha * tf.reduce_sum(penalty_lambda * tf.nn.l2_loss(w_2)) \
            + lasso_alpha * tf.reduce_sum(penalty_lambda * tf.nn.l2_loss(w_1)) \
            + (1 - lasso_alpha) * tf.reduce_sum(penalty_lambda * tf.abs(w_6)) \
            + (1 - lasso_alpha) * tf.reduce_sum(penalty_lambda * tf.abs(w_5)) \
            + (1 - lasso_alpha) * tf.reduce_sum(penalty_lambda * tf.abs(w_4)) \
            + (1 - lasso_alpha) * tf.reduce_sum(penalty_lambda * tf.abs(w_3)) \
            + (1 - lasso_alpha) * tf.reduce_sum(penalty_lambda * tf.abs(w_2)) \
            + (1 - lasso_alpha) * tf.reduce_sum(penalty_lambda * tf.abs(w_1))
