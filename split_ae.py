...
class split_ae:
    ...
    def __init__(self, x, y, dh, lr):

        with tf.name_scope("Input_Layer"):
            self.X, self.Y, self.dim_h, self.learning_rate = x, y, dh, lr
    ...
    def initiate(self):
        ...
        with tf.Session() as sess:
            ...
            for iter in range(5001):
                c, _, self.W, self.B = sess.run([self.C, self.O, self.W_enc, self.B_enc],
                                                feed_dict={self.input_data: self.X})
                if iter == 0: train_cost = c
                if iter % 100 == 0:
                    print(iter, "th step:\t", "Cost is ", c)

            test_cost = sess.run(self.C, feed_dict={self.input_data: self.Y})
            c = (train_cost - test_cost) / train_cost
            print("Testing Phase: ", c)
